...
/**
 * PurchaseOrderCreate — Create a new purchase order page.
 *
 * TODO:
 *  - Fetch suppliers list from API for dropdown selection
 *  - Fetch products list from API for item selection
 *  - Implement form to add PO items (product, qty, unit cost)
 *  - Calculate subtotal, discount, tax, total dynamically
 *  - Submit via createPurchaseOrder API
 *  - Handle validation and error states
 *  - Navigate to /purchase-orders/:id after creation
 */
...
export default function PurchaseOrderCreate() {
  ...
  // TODO: Replace with actual data fetch (suppliers, products)
  useEffect(() => {
    // Simulate loading
  ...
  return (
    <div className="space-y-6 p-6 bg-white rounded-3xl shadow-sm dark:bg-gray-800 dark:shadow-none">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400">
        ...
        <span>/</span>
        <span className="text-gray-600 dark:text-gray-300">Create PO</span>
      </nav>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Create Purchase Order</h1>
            <p className="text-sm text-gray-400 dark:text-gray-400 mt-0.5">Add a new purchase order to your inventory</p>
          </div>
        </div>
        <button
          onClick={() => navigate("/purchase-orders")}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <p className="text-sm text-gray-400">Preparing form...</p>
        </div>
      ) : (
        <div className="rounded-2xl border border-[#ececf2] dark:border-gray-700 bg-white dark:bg-gray-800 p-8">
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-400 max-w-md">
              The purchase order creation form is coming soon. You will be able to select a supplier, add items with quantities and pricing, and submit the PO.
            </p>

            {/* TODO: Replace with actual create form */}
            <div className="mt-8 w-full max-w-lg rounded-2xl bg-gradient-to-r from-[#f8f8fc] dark:from-gray-800/80 to-white dark:to-gray-800/60 p-6 border border-dashed border-[#ececf2] dark:border-gray-700">
              ...
              <div className="space-y-3">
                {["PO Number", "Supplier", "Order Date", "Expected Date", "Items Table (x3)", "Discount & Tax"].map((section) => (
            ...
            <div className="flex items-start gap-3 mt-8 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 max-w-lg">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                <strong className="font-semibold">TODO:</strong> Fetch suppliers and products from API, build dynamic item rows, calculate totals, submit via <code className="text-xs bg-amber-100 dark:bg-amber-900/40 px-1 rounded">createPurchaseOrder()</code> API.
              </p>
            </div>
          </div>
        </div>
