...
export default function AppSidebar() {
  ...
  // Hotfix 11.8.3: Detect if user is already on Cash Register page
  const isCashRegisterPage = location.pathname === "/cash-register";
  ...
  const handleLogout = async () => {
    try {
      ...
      // Hotfix 11.8.1: Show dialog when shift is still open
      if (status === 409 && code === "SHIFT_STILL_OPEN") {
        setLogoutError({
          title: "Cash Register Masih Aktif",
          message:
            "Anda masih memiliki Shift yang belum ditutup.\n\n" +
            "Untuk menjaga konsistensi transaksi dan saldo kas, " +
            "silakan lakukan Close Shift terlebih dahulu sebelum Logout.",
        });
        return;
      }
    }
  ...
  const handleNavigateToCashRegister = () => {
    navigate("/cash-register");
    setLogoutError(null);
  };

  const handleDismissLogoutError = () => {
    setLogoutError(null);
  };
  ...
  return (
    <>
      ...
      {/* Hotfix 11.8.1/11.8.3: Context-aware shift still open dialog */}
      <Dialog
          ...
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-lg font-semibold">
                {logoutError?.title || "Cash Register Masih Aktif"}
              </DialogTitle>
            </div>
          </DialogHeader>
          <DialogDescription className="whitespace-pre-line text-sm text-gray-600 dark:text-gray-400">
            {logoutError?.message}
          </DialogDescription>
          <DialogFooter>
            <Button variant="outline" onClick={handleDismissLogoutError}>
              {isCashRegisterPage ? "OK" : "Batal"}
            </Button>
            {!isCashRegisterPage && (
              <Button onClick={handleNavigateToCashRegister}>
                Ke Cash Register
              </Button>
            )}
          </DialogFooter>
