...
export default function useReports() {
  ...
  const formatRupiah = (value) =>
    value == null ? "Rp 0" : "Rp " + Number(value).toLocaleString("id-ID");
  ...
  const fetchAll = useCallback(async (startDate, endDate) => {
    ...
    try {
      ...
      setSales({
        ...
        total_returns: sBody.total_returns ?? 0,
        total_returns_count: sBody.total_returns_count ?? 0,
        net_revenue: sBody.net_revenue ?? sBody.total_revenue ?? 0,
  ...
  // ── Date range setter ───────────────────────────────────────────────
  const setDateRange = useCallback((date_from, date_to) => {
    setDateRangeState({
      date_from: date_from || "",
      date_to: date_to || "",
    });
